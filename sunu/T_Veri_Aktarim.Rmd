...
-   Değişken adi boşluklu yazılmışsa ne olur?
...
-   Eksik veri boyunca ayni şekilde girilmelidir.
...
-   Tercihimiz .csv uzantılı veriler ama büyük veri setleri az yer kalması icin .txt,.prn formatında karsımıza çıkabilmektedir.
...
csv_dosya <- read.csv("path to the file/filename.csv", 
                      header = TRUE)

# tab ile ayrılmış txt dosyaları
txt_dosya <- read.table("path to the file/filename.txt", h
                        eader = TRUE, sep = "\t")
```
...
- `r emo::ji('link')`[verifwf.txt](import/verifwf.txt)
...
**write.csv()** fonksiyonu kullanılarak yazılan veri dosyaları "i" ile,   **write.csv2()**  fonksiyonu kullanılarak yazılan veri dosyaları ise "," ile ayrılır  iki fonksiyonun bir diğer farkı ise ondalık sayı ayıracıdır.
...
- **dump()** fonksiyonu nesneyi ayni işlemle çalışma alanına getirirken,
