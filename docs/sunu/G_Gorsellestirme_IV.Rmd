...
library(tuev)
data(PISA_OGR_2018)
grafik_1 <- ggplot(PISA_OGR_2018,aes(x=ODOKUMA1))
```
]
...
ggplot( PISA_OGR_2018 %>% group_by(SINIF) %>% mutate(ort=mean(ODOKUMA1)) %>% ungroup(),
  ...
  scale_y_continuous(labels = scales::percent)+
    scale_colour_discrete("Cinsiyet", breaks=c("1","2"), labels=c("Kiz", "Erkek"))
...
- olsuturalan grafik **p** nesnesine atanmıştır. **p** nesnesine **+** ile katmanlar eklenebilir.
- Kategorik degiskenler eksen değerlerini belirler.
...
<!-- ```{r} -->
<!-- df <- PISA_OGR_2018 %>% group_by(OKUL_TUR)%>% sample(5) %>% ungroup() -->
